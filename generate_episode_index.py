...
class EpisodeIndexGenerator:
    ...
    def __init__(
        ...
        s3_key_prefix: str = "",
        local_snapshots_dir: str = "./zorkgpt/snapshots",
    ):
        ...
        try:
            ...
            for page in pages:
                ...
                for obj in page["Contents"]:
                    ...
                    match = re.match(
                        rf"{re.escape(snapshots_prefix)}([^/]+)/turn_(\d+)\.json$", key
                    )
                    if match:
    ...
    def _get_s3_episode_info(self, episode_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            episode_info = {
                ...
                "source": "s3",
                "start_time": first_snapshot.get("metadata", {}).get(
                    "timestamp", episode_id
                ),
                "end_time": last_snapshot.get("metadata", {}).get("timestamp", ""),
                "total_turns": last_snapshot.get("metadata", {}).get(
                ...
                "death_count": last_snapshot.get("current_state", {}).get(
                    "death_count", 0
                ),
    ...
    def _get_s3_snapshot(self, key: str) -> dict[str, Any] | None:
    ...
    def _get_local_episode_info(
        self, episode_id: str, episode_dir: str
    ) -> dict[str, Any] | None:
    ...
    def _read_local_snapshot(self, file_path: str) -> dict[str, Any] | None:
    ...
    def save_index(
        self, index: dict[str, Any], output_file: str = "./zorkgpt/episodes.json"
    ) -> None:
    ...
    def upload_index_to_s3(
        self, index: dict[str, Any], s3_key: str = "episodes.json"
    ) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--s3-prefix", default=config.get("s3_key_prefix", ""), help="S3 key prefix"
    )
    parser.add_argument(
        "--local-dir", default="./zorkgpt/snapshots", help="Local snapshots directory"
    )
    parser.add_argument(
        "--output", default="./zorkgpt/episodes.json", help="Output file path"
    )
    parser.add_argument(
        "--upload-s3", action="store_true", help="Upload index to S3 after generating"
    )
    ...
    elif config.get("s3_key_prefix") and args.s3_prefix == config.get(
        "s3_key_prefix", ""
    ):
    ...
    generator = EpisodeIndexGenerator(
        s3_bucket=s3_bucket, s3_key_prefix=s3_prefix, local_snapshots_dir=args.local_dir
    )
    ...
    if index["episodes"]:
        ...
        print(
            f"Date range: {index['episodes'][-1]['start_time']} to {index['episodes'][0]['start_time']}"
        )
