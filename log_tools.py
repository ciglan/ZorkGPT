...
def main():
    ...
    render_parser = subparsers.add_parser(
        "render", help="Render JSON logs as human-readable text"
    )
    render_parser.add_argument("input_file", help="JSON log file to parse")
    render_parser.add_argument(
        "-o", "--output", help="Output file (stdout if not specified)"
    )
    ...
    rl_parser = subparsers.add_parser(
        "rl", help="Format logs for reinforcement learning"
    )
    ...
    rl_parser.add_argument(
        "-e", "--episode", type=int, help="Specific episode to extract (default: all)"
    )
    ...
    episodes_parser = subparsers.add_parser(
        "episodes", help="Show episode details in markdown table format"
    )
    ...
    episodes_parser.add_argument(
        "--limit", type=int, help="Limit the number of episodes shown (default: all)"
    )
    episodes_parser.add_argument(
        "--min-score", type=int, help="Only show episodes with score >= this value"
    )
    ...
    try:
        ...
        if args.command == "render":
            ...
            print(
                f"Average turns per episode: {total_turns / episode_count if episode_count else 0:.2f}"
            )
            print(
                f"Average reward per episode: {total_rewards / episode_count if episode_count else 0:.2f}"
            )
            ...
            for episode in episodes_data.values():
                episode["runtime"] = calculate_runtime(
            ...
            if args.min_score is not None:
                episodes_list = [
                    ep for ep in episodes_list if ep["score"] >= args.min_score
                ]
            ...
            if not episodes_list:
                ...
                for episode in episodes_list:
                    ...
                    agent_model = (
                        episode["agent_model"][:40] + "..."
                        if len(episode["agent_model"]) > 40
                        else episode["agent_model"]
                    )
                    critic_model = (
                        episode["critic_model"][:40] + "..."
                        if len(episode["critic_model"]) > 40
                        else episode["critic_model"]
                    )
                    info_ext_model = (
                        episode["info_ext_model"][:40] + "..."
                        if len(episode["info_ext_model"]) > 40
                        else episode["info_ext_model"]
                    )

                    score_display = (
                        f"{episode['score']}/{episode['max_score']}"
                        if episode["max_score"] > 0
                        else str(episode["score"])
                    )
                ...
                if len(episodes_list) > 1:
                    avg_score = sum(ep["score"] for ep in episodes_list) / len(
                        episodes_list
                    )
                    avg_turns = sum(ep["turns"] for ep in episodes_list) / len(
                        episodes_list
                    )
                    avg_reward = sum(ep["total_reward"] for ep in episodes_list) / len(
                        episodes_list
                    )
